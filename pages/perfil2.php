...
<body>
    ...
    <script>
        ...
        function cargarFavoritos() {
            $('#favoritosContainer').html('<div class="col-12 text-center py-5"><div class="spinner-border text-primary"></div></div>');
            
            $.get('../api/obtener_favoritos.php', function(data) {
                if (data.success && data.favoritos.length > 0) {
                        ...
                        html += `
                            <div class="col-md-4">
                                <div class="favorito-card">
                                    ${fav.imagen ? `<img src="../uploads/${fav.imagen}" class="lugar-imagen">` : ''}
                                    <h5><i class="bi bi-geo-alt-fill text-danger"></i> ${fav.nombre}</h5>
                                    <p class="text-muted mb-2"><small>${fav.categoria}</small></p>
                                    <p>${fav.descripcion.substring(0, 100)}...</p>
                                    <button class="btn btn-danger btn-sm btn-block" onclick="quitarFavorito(${fav.id_lugar})">
                                        <i class="bi bi-heart-fill"></i> Quitar de Favoritos
                                    </button>
                                    <a href="../lugar.php?id=${fav.id_lugar}" class="btn btn-primary btn-sm btn-block">
                    ...
                    $('#favoritosContainer').html(`
                        <div class="col-12 empty-state">
                            ...
                            <a href="../index.php" class="btn btn-primary">Explorar Lugares</a>
                        </div>
                    `);
                }
            }).fail(function() {
                $('#favoritosContainer').html('<div class="col-12 alert alert-danger">Error al cargar favoritos</div>');
            });
        }
        ...
        function quitarFavorito(idLugar) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('../api/toggle_favorito.php', { id_lugar: idLugar }, function(data) {
                        if (data.success) {
                            Swal.fire('¡Eliminado!', 'Lugar quitado de favoritos', 'success');
                            cargarFavoritos();
                        }
                    });
                }
                    ...
                    data.seguidores.forEach(seg => {
                        html += `
                            <div class="user-card">
                                <div class="row align-items-center">
                                    <div class="col-auto">
                                        <img src="${seg.imagen_perfil || 'https://ui-avatars.com/api/?name=' + seg.nombre}" class="user-avatar-small">
                                    </div>
                                    <div class="col">
